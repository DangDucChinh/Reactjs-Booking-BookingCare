...
class UserRedux extends Component {
    ...
    async componentDidMount() {
        try {
            let res = await getAllCodeService('gender'); // vì api của server nhận tham số đó là type của url 
            // chính vì vậy tại đây ta cần truyền cho nó một tham số query để nó get được allcode tương ứng .   

            if(res && res.errCode === 0){
                this.setState({
                    genderArr : res.allcodes
                })
            }
        } catch (error) {
            console.log(error);
